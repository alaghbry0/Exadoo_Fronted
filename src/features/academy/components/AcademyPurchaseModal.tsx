...
type OpenSubscribeUnified =
  ...
  | { productType: 'bundle'; productId: string; title?: string; price?: string; telegramUrl?: string | null };
...
export default function AcademyPurchaseModal() {
  ...
  const [bundleTelegramUrl, setBundleTelegramUrl] = useState<string | null>(null); // للحِزم فقط
  ...
  const closeAll = useCallback(() => {
    ...
    window.dispatchEvent(new CustomEvent('modal:close', { detail: { name: 'academyPurchase' } }));
  ...
  useEffect(() => {
    const onOpen = (e: Event) => {
      ...
      const detail: OpenSubscribeUnified | OpenSubscribeLegacy | undefined = ce.detail;
      if (!detail) return;
      ...
      setOpen(true);
      window.dispatchEvent(new CustomEvent('modal:open', { detail: { name: 'academyPurchase' } }));
    };

    window.addEventListener('open-subscribe' as any, onOpen as any);
    return () => window.removeEventListener('open-subscribe' as any, onOpen as any);
  }, []);
  ...
  const priceInfo = useMemo(() => {
    if (!selected && !priceOverride) return { isFree: false, price: 0, priceStr: '' };

    let pv: string | undefined = priceOverride || (selected?.price as string | undefined);
    // للكورسات: خذ الخصم إن موجود
    if (!priceOverride && selected && 'discounted_price' in selected && selected.discounted_price) {
  ...
  return (
    <>
          ...
          <ScrollArea className="flex-1">
            <div className="space-y-8 p-4 pt-6 pb-12 text-right">
              ...
              <div className="bg-gradient-to-br from-primary-500 to-primary-600 rounded-2xl p-6 text-white shadow-lg text-center relative overflow-hidden">
                ...
                <p className="font-medium text-white/80 mb-1 z-10 relative">{rightBadge}</p>
                ...
                <ul className="space-y-3">
                  <li className="flex items-start">
                    <span className="text-gray-600 leading-relaxed text-sm">
                      {kind === 'course' ? 'وصول كامل للدورة ومحتواها' : 'وصول لجميع الدورات داخل الحزمة'}
                    </span>
                  </li>
                  <li className="flex items-start">
                    <span className="text-gray-600 leading-relaxed text-sm">
      ...
      {paymentStatus === 'exchange_success' && (
        <PaymentExchangeSuccess onClose={handleSuccessClose} planName={displayTitle} />
      )}
