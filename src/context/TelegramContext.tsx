...
export const TelegramProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const initializeTelegram = useCallback(() => {
        ...
        // ✅ فحص وجود initDataUnsafe قبل الوصول إليه
        if (tg.initDataUnsafe) {
            const userId = tg.initDataUnsafe.user?.id?.toString() || null;
            if (userId) {
                console.log("✅ Telegram User ID من initDataUnsafe:", userId);
                setTelegramId(userId);
                sessionStorage.setItem("telegramId", userId);
                localStorage.setItem("telegramId", userId);
                setIsTelegramReady(true);
            } else {
                console.warn("⚠️ لم يتم العثور على معرف المستخدم في initDataUnsafe.");
            }
        } else {
            console.warn("⚠️ initDataUnsafe غير متاح في Telegram WebApp.");
            // ✅ هنا يمكننا اختيار عدم مسح telegramId إذا لم يتوفر initDataUnsafe
            // ✅ أو يمكننا محاولة استرجاع telegramId من التخزين فقط
            const storedTelegramId = sessionStorage.getItem("telegramId") || localStorage.getItem("telegramId");
            if (storedTelegramId) {
                console.log("✅ تم استرجاع telegram_id من التخزين (initDataUnsafe غير متاح):", storedTelegramId);
                setTelegramId(storedTelegramId);
                setIsTelegramReady(true);
            } else {
                console.warn("⚠️ لم يتم العثور على telegram_id في التخزين أيضًا.");
                // ✅ في هذه الحالة، يمكننا اختيار ترك telegramId فارغًا أو تعيينه إلى null
                setTelegramId(null); // أو يمكنك اختيار عدم تغييره إذا كنت تفضل ذلك
            }
        }
