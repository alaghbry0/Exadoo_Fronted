...
export const getUserJettonWallet = async (userTonAddress: string) => {
  try {
    const response = await fetch(
      `https://tonapi.io/v2/accounts/${userTonAddress}/jettons`,
    );
    if (!response.ok) {
    ...
    console.log(
      `✅ User USDT Wallet Address: ${usdtJetton.wallet_address.address}`,
    );
...
export const getBotJettonWallet = async (botTonAddress: string) => {
  try {
    const response = await fetch(
      `https://tonapi.io/v2/accounts/${botTonAddress}/jettons`,
    );
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    ...
    console.log(
      `✅ Bot USDT Wallet Address: ${usdtJetton.wallet_address.address}`,
    );
    return usdtJetton.wallet_address.address;
  } catch (error) {
...
export const handleTonPayment = async (
  ...
  try {
    ...
    const recipientJettonWalletAddress =
      await getBotJettonWallet(botWalletAddress);
