...
import { createPaymentIntent } from "@/domains/payments/utils/paymentIntent";
import { handleTonPayment } from "@/domains/payments/utils/tonPayment";
import type { ProductType } from "@/domains/payments/types";
...
export function useServicePayment() {
  ...
  const startPollingPaymentStatus = useCallback(
    (paymentToken: string, onExchangeSuccess?: () => void) => {
  ...
  const payWithStars = useCallback(async (_input: ServicePaymentInput) => {
    showToast.error(
      "استخدم useTelegramPayment لدفعات Stars (تم توحيده مسبقًا).",
    );
    return false;
  }, []);
