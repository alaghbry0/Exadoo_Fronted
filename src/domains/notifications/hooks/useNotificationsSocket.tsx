...
export function useNotificationsSocket(
  ...
  const connect = useCallback(() => {
    if (!telegramId || !isMounted.current) {
      if (!telegramId)
        console.log("ℹ️ WebSocket connect aborted: telegramId is null.");
      if (!isMounted.current)
        console.log("ℹ️ WebSocket connect aborted: component unmounted.");
      return;
    }
    ...
    if (socketRef.current) {
      console.log(
        "ℹ️ Closing existing WebSocket connection before reconnecting.",
      );
      try {
    ...
    try {
      ...
      const connectionTimeout = setTimeout(() => {
        if (socket.readyState !== WebSocket.OPEN && isMounted.current) {
          console.warn(
            `⚠️ WebSocket connection attempt timed out after 10 seconds. URL: ${socketUrl}`,
          );
      ...
      socket.onmessage = (event) => {
        ...
        try {
          ...
          if (data.type === "new_notification") {
            ...
            const readNotificationId = (data.data as ReadNotificationData)
              ?.notification_id;
            if (readNotificationId) {
              queryClient.setQueryData(
                ["unreadNotificationsCount", telegramId],
                (oldCount: number | undefined) =>
                  Math.max(0, (oldCount || 0) - 1),
              );
              ...
              queryClient.setQueryData(
                ...
                (
                  oldData:
                    | {
                        pages: Array<
                          Array<{ id: string; read_status: boolean }>
                        >;
                      }
                    | undefined,
                ) => {
          ...
          console.error(
            "❌ Error parsing WebSocket message or updating cache:",
            error,
            "Raw data:",
            event.data,
          );
      ...
      socket.onclose = (e) => {
        ...
        console.log(
          `🔌 WebSocket closed. Code: ${e.code}, Reason: '${e.reason}', Was Clean: ${e.wasClean}`,
        );
        ...
        if (e.code !== 1000 && e.code !== 1001) {
          if (reconnectAttemptsRef.current < maxReconnectAttempts) {
            ...
            console.log(
              `🔄 Reconnecting WebSocket... Attempt ${reconnectAttemptsRef.current}/${maxReconnectAttempts} in ${delay / 1000}s`,
            );
            ...
            console.log(
              `🚫 Max WebSocket reconnect attempts reached (${maxReconnectAttempts}). Giving up.`,
            );
          }
        } else {
    ...
    (message: NotificationMessage) => {
      if (socketRef.current?.readyState === WebSocket.OPEN) {
        ...
        if (connectionState === "disconnected") {
          console.log(
            "💡 Attempting to connect WebSocket due to pending message.",
          );
          connect();
        }
        return false;
  ...
  useEffect(() => {
    if (telegramId && isMounted.current) {
      // تأكد من أن المكون ما زال mounted
      connect();
    } else if (!telegramId) {
      // إذا تم تعيين telegramId إلى null (مثل تسجيل الخروج)
      if (socketRef.current) {
        console.log(
          "ℹ️ Closing WebSocket connection due to telegramId becoming null.",
        );
    ...
    return () => {
      console.log(
        "ℹ️ Cleaning up WebSocket connection on component unmount or telegramId change.",
      );
      if (reconnectTimeoutRef.current) {
        clearTimeout(reconnectTimeoutRef.current);
        reconnectTimeoutRef.current = null;
