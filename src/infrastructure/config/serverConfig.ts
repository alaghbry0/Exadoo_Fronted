...
export function resolveBackendConfig(
  ...
  const baseUrl =
    cleanEnvValue(process.env.BACKEND_BASE_URL) ??
    cleanEnvValue(process.env.NEXT_PUBLIC_BACKEND_URL);
  ...
  if (!secret) {
    missing.push(
      "SECRET or WEBHOOK_SECRET (NEXT_PUBLIC_WEBHOOK_SECRET as a fallback)",
    );
  }
