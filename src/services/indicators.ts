...
export async function fetchIndicators(telegramId: string): Promise<IndicatorsData> {
  const r = await fetch(`/api/indicators?telegramId=${encodeURIComponent(telegramId)}`)
