...
export async function fetchConsultancy(telegramId: string): Promise<ConsultancyData> {
  const r = await fetch(`/api/consultancy?telegramId=${encodeURIComponent(telegramId)}`)
