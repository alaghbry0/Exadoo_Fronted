  ...
  "group block overflow-hidden rounded-3xl transition-all duration-300 focus:outline-none",
  {
    variants: {
      variant: {
        ...
        dark: "text-white bg-gradient-to-br from-[#0B1220] via-[#0E1A2F] to-[#0B1220] hover:-translate-y-0.5 focus-visible:ring-4 focus-visible:ring-primary/30",
        compact:
  ...
  const Content = (
    ...
    >
      ...
      {variant === "split" ? (
        ...
        <div
          className={cn("relative p-5 md:p-6", variant === "compact" && "p-5")}
        >
          <div
            className={cn(
              "flex items-start gap-4",
              layout === "wide" ? "sm:items-center sm:justify-between" : "",
            )}
          >
            ...
            {layout === "wide" && (
              <div className="mt-4 sm:mt-0 sm:mr-6 shrink-0">
                {rightSlot ? (
                  ...
                  <CTA
                    label={ctaLabel}
                    tone={variant === "dark" ? "light" : "primary"}
                  />
                )}
              </div>
            )}
  ...
  return (
    <div className="flex items-start gap-4">
      ...
      <div className="flex-1">
        <TitleBlock title={title} badge={badge} titleId={titleId} />
        {desc && (
          <p id={descId} className="mt-2 text-gray-600 dark:text-gray-300">
            {desc}
          </p>
        )}
      </div>
...
function TitleBlock({
  ...
  return (
    <div className="flex items-center gap-2">
      <h3
        id={titleId}
        className="text-xl md:text-2xl font-bold text-gray-900 dark:text-neutral-100 truncate"
      >
        {title}
      </h3>
      {badge && (
        <span className="px-2 py-0.5 text-xs font-semibold bg-primary-100 text-primary-700 rounded-full whitespace-nowrap">
          {badge}
        </span>
      )}
    </div>
...
function CTAInline({
  ...
  return (
    <div
      className={cn(
        "text-sm font-semibold flex items-center gap-1 transition-all duration-200 group-hover:gap-2",
        text,
        className,
      )}
    >
