...
export default async function handler(
  ...
  try {
    ...
    if (h) {
      try {
        extraHeaders = JSON.parse(b64urlDecode(h) || "{}");
      } catch {}
    }
    ...
    for (const [k, v] of upstream.headers.entries()) {
      if (
        /^content-(type|length|range|encoding|disposition)|^accept-ranges|^etag|^cache-control/i.test(
          k,
        )
      ) {
    ...
    const isPlaylist =
      /application\/vnd\.apple\.mpegurl|audio\/mpegurl|application\/x-mpegurl|\.m3u8(\?|#|$)/i.test(
        ct,
    ...
    if (upstream.headers.get("accept-ranges")) {
      res.setHeader(
        "Accept-Ranges",
        upstream.headers.get("accept-ranges") as string,
      );
    }
    if (upstream.headers.get("content-range")) {
      res.setHeader(
        "Content-Range",
        upstream.headers.get("content-range") as string,
      );
    }
