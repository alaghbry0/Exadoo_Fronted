...
export default async function handler(
  ...
  const { telegram_id, amount, payment_token, title, description, payload } =
    req.body || {};

  if (!payment_token || !telegram_id || amount === undefined) {
    return res.status(400).json({
      error: "Missing required fields: telegram_id, amount, payment_token",
    });
  }
  ...
  try {
    ...
    if (!response.ok || !data?.ok || !data?.result) {
      return res.status(502).json({
        ...
        details: data?.description || "unknown",
      });
    }
