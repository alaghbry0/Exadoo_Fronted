...
function normalizeConsultancyPayload(payload: any): ConsultancyData {
  ...
  const c =
    root.consultancy && typeof root.consultancy === "object"
      ? root.consultancy
      : root;
  ...
  const details: ConsultancyDetails = {
    ...
    instructor_image:
      det.instructor_image == null ? null : S(det.instructor_image),
    about_instructor: S(det.about_instructor),
    description: S(det.description),
    options: parseJSON<string[]>(det.options, []),
    price: S(det.price, "0"),
    discounted_price:
      det.discounted_price == null ? null : S(det.discounted_price),
    session_minutes: S(det.session_minutes, "30"),
    time_availability: parseJSON(det.time_availability, []) as Array<{
      from: string;
      to: string;
    }>,
  ...
  const my_consultations: MyConsultation[] = A(c.my_consultations).map(
    (m: any) => ({
      id: S(m.id),
      user_id: S(m.user_id),
      date: S(m.date),
