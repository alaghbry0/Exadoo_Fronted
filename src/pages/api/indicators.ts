...
const normalizePayload = (payload: any): IndicatorsData => {
  ...
  const bi =
    root.buy_indicators && typeof root.buy_indicators === "object"
      ? root.buy_indicators
      : root;

  const my_subscription = bi.my_subscription
    ? normalizeMySub(bi.my_subscription)
    : null;
  const subscriptions = Array.isArray(bi.subscriptions)
    ? bi.subscriptions.map(normalizePlan)
    : [];
