...
export interface Course {
  id: string; title: string; short_description: string; description: string;
  outcomes: string[]; language: string; category_id: string; sub_category_id: string;
  section: string; requirements: string[]; price: string; discount_flag: string | null;
  discounted_price: string; level: Level; user_id: string; thumbnail: string; video_url: string;
  expiry_days: string; date_added: string; last_modified: string; course_type: string;
  is_top_course: string; is_highlight_course: string; is_admin: string; is_disabled: string;
  status: Status; course_overview_provider: string; meta_keywords: string; meta_description: string;
  is_free_course: string | null; is_limit_working: string; multi_instructor: string; creator: string;
  cover_image: string; rating: number; number_of_ratings: number; instructor_name: string;
  total_enrollment: number; total_number_of_lessons: number; completion?: number;
  total_number_of_completed_lessons?: number;
}

export interface Bundle {
  id: string; title: string; description: string; price: string; image: string;
  requirements: string[]; outcomes: string[]; course_ids: string; free_sessions_count: string;
  discounted_price: string; cover_image: string; telegram_url: string; status: string;
  sub_category_id: string; is_top: string; is_highlight: string; is_disabled: string; date_added: string;
}

export interface Category {
  id: string; code: string; name: string; parent: string; type: string | null; slug: string;
  date_added: string; last_modified: string; font_awesome_class: string; thumbnail: string;
...
export interface AcademyData {
  ...
  top_course_ids: string[]; highlight_course_ids: string[];
  top_bundle_ids: string[]; highlight_bundle_ids: string[];
  categories: Category[];
}
...
const idsFromMixed = (arr: any[]): string[] => {
  ...
  return (arr || []).map((x) => (typeof x === 'object' && x && 'id' in x ? S((x as any).id) : S(x))).filter(Boolean)
}
...
function normalizeCourse(raw: any): Course {
  return {
    ...
    total_number_of_completed_lessons:
      raw.total_number_of_completed_lessons != null ? N(raw.total_number_of_completed_lessons, 0) : undefined,
  }
}
...
function normalizeAcademyPayload(payload: any): AcademyData {
  ...
  const ac = root.academy && typeof root.academy === 'object' ? root.academy : root

  // 2) ادعم الحقلين: all_* أو * بدون all_
  const rawCourses = Array.isArray(ac.all_courses) ? ac.all_courses : (Array.isArray(ac.courses) ? ac.courses : [])
  const rawBundles = Array.isArray(ac.all_bundles) ? ac.all_bundles : (Array.isArray(ac.bundles) ? ac.bundles : [])
  ...
  const top_course_ids = idsFromMixed(ac.top_courses ?? ac.top_course_ids ?? [])
  const highlight_course_ids = idsFromMixed(ac.highlight_courses ?? ac.highlight_course_ids ?? [])
  const top_bundle_ids = idsFromMixed(ac.top_bundles ?? ac.top_bundle_ids ?? [])
  const highlight_bundle_ids = idsFromMixed(ac.highlight_bundles ?? ac.highlight_bundle_ids ?? [])

  // 4) الاشتراكات: يدعم الشكلين
  const me = ac.my_enrollments ?? {}
  let course_ids: string[] = Array.isArray(me.course_ids) ? me.course_ids.map(String) : []
  let bundle_ids: string[] = Array.isArray(me.bundle_ids) ? me.bundle_ids.map(String) : []
  if (Array.isArray(me.courses) && !course_ids.length) course_ids = idsFromMixed(me.courses)
  if (Array.isArray(me.bundles) && !bundle_ids.length) bundle_ids = idsFromMixed(me.bundles)
