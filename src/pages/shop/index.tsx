...
const WideIndicators = React.memo<{ meta: TileMeta }>(({ meta }) => {
  ...
  const priceOld =
    lifetime?.discounted_price && lifetime.discounted_price !== lifetime.price ? lifetime.price : undefined
  const discount =
    priceOld && priceNow ? Math.round(((Number(priceOld) - Number(priceNow)) / Number(priceOld)) * 100) : null

  return (
    <Link
      href={meta.href}
      className={cn('group block rounded-card-lg outline-none', ringFocus(meta.accent))}
      aria-label={meta.title}
      prefetch={false}
    >
      ...
      <CardContent className="p-5 md:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="shrink-0">
            <Button variant="outline" className="rounded-xl w-full sm:w-auto border-gray-500 dark:border-neutral-700 font-semibold">
              احجز الآن
...
export default function ShopHome() {
  ...
  const renderUnifiedHit = (hit: any) => {
    ...
    if (doc.kind === 'course') {
      return (
        <Link key={`c-${doc.id}`} href={doc.href} className="group block col-span-12 sm:col-span-6 lg:col-span-4" prefetch={false}>
          <Card className="rounded-card-lg bg-white border border-gray-100 shadow-card dark:bg-neutral-900 dark:border-neutral-800">
            <CardContent className="p-4">
              <div className="text-sm text-gray-500 mb-1">دورة</div>
              <div className="font-bold text-gray-900 dark:text-neutral-100 line-clamp-1">{doc.title}</div>
              <div className="text-sm text-gray-600 dark:text-neutral-300 line-clamp-2 mt-1">{doc.subtitle}</div>
    ...
    if (doc.kind === 'bundle') {
      return (
        <Link key={`b-${doc.id}`} href={doc.href} className="group block col-span-12 sm:col-span-6 lg:col-span-4" prefetch={false}>
          <Card className="rounded-card-lg bg-white border border-gray-100 shadow-card dark:bg-neutral-900 dark:border-neutral-800">
            <CardContent className="p-4">
    ...
    if (doc.kind === 'category') {
      return (
        <Link key={`g-${doc.id}`} href={doc.href} className="group block col-span-12 sm:col-span-6 lg:col-span-3" prefetch={false}>
          <Card className="rounded-card-lg bg-white border border-gray-100 shadow-card dark:bg-neutral-900 dark:border-neutral-800">
            <CardContent className="p-4">
              <div className="text-xs text-gray-500">تصنيف</div>
              <div className="font-bold text-gray-900 dark:text-neutral-100 line-clamp-1 mt-1">{doc.title}</div>
              <div className="mt-3 text-primary-600 dark:text-primary-400 text-sm font-semibold flex items-center gap-1">
      ...
      <main className="max-w-6xl mx-auto px-4 pb-12">
        ...
        <section className="text-center pt-20 pb-12" aria-labelledby="page-title">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <h1 id="page-title" className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-neutral-100 mb-4 leading-tight font-display">
              متجر Exaado
            </h1>
        ...
        {/* Search input */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.4 }} className="mb-8 max-w-2xl mx-auto" role="search">
